...
Eshell overrides the map after initialization therefore we have to set it after.
#+begin_src emacs-lisp
(add-hook 'eshell-mode-hook #'(lambda ()
                                (local-set-key (kbd "<home>") #'eshell-bol))) ;; Avoid cursor going before prompt
#+end_src
...
**** FIXME: font-lock disable

Somehow disabling font-lock-mode fixes the prompt not coloring properly.
#+begin_src emacs-lisp
(add-hook 'eshell-mode-hook (lambda () (font-lock-mode -1))) 
#+end_src
