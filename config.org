...
+ Fix `input-decode-map' updating too early if evaluated later it works properly. This is only an issue when emacs runs as a daemon.
...
This only works on svn repositories with internet! Without internet it will hang.
...
(use-package auto-dark
  ...
  ;; TODO this cannot read the appropriate variables when running in daemon mode.
  (if (string-match-p "GNOME" (getenv "XDG_CURRENT_DESKTOP")) ;; Try to enable only for GNOME
      (auto-dark-mode 1)))
#+end_src
...
(use-package evil
  ...
  ;; `evil-respect-visual-line-mode' won't affect arrow keys. So let's set it manually. This has to be set that way so keybinding from other modes (such as undoo-tree) do not get overriden.
...
(defhydra hydra-expand-region (global-map "C-+") ;; For emacs state
  "expand region"
  ("+" er/expand-region)
  ("-" er/contract-region))

(define-key evil-motion-state-map (kbd "+") #'hydra-expand-region/body)
#+end_src
...
(use-package org-roam
  ...
  :config
  ;; If you're using a vertical completion framework, you might want a more informative completion interface. Propertize the icon with the `all-the-icons' package icons instead of relying on the inherited font.
  (setq org-roam-node-display-template (concat "${title:50}  " (propertize (all-the-icons-faicon "tags") 'face `(:inherit org-tag :family ,(all-the-icons-faicon-family))) (propertize " ${tags:*}" 'face 'org-tag)))
...
;; TODO find a way to use FontAwesome for the icons
;; (esh-section esh-folder
;;              (check-empty-dir)
;;              ""
;;              '(:inherit outline-3 :family "FontAwesome" :underline t))

;; (esh-section esh-dir
;;              "" ;; Icon left to `esh-folder'
;;              (let ((current-dir (abbreviate-file-name (eshell/pwd))))
;;                (if (< (string-width current-dir) 103)
...
;; Variables
(setq compilation-scroll-output t ;; Scroll to botom of compilation screen unless pointer moves
...
(use-package projectile
  ...
  (setq projectile-track-known-projects-automatically nil
        ;; If you don't have internet it will hang on SVN default command. Change it for the generic command which does not require internet.
...
(use-package writeroom-mode
  :config
  (setq writeroom-global-effects (remq 'writeroom-set-fullscreen writeroom-global-effects))) ;; Do not set fullscreen on activation
#+end_src
...
+ [X] Winner undo
+ [X] Change focus
+ [ ] Decide if `completion-ignored-extensions' should be empty to show all files when completing.
+ [ ] Popper
+ [ ] Shackles
